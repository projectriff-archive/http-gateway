...
func NewKafkaProducer(brokerAddrs []string) (*producer, error) {
...
func (p *producer) Send(topic string, message dispatcher.Message) error {
	kafkaMsg, err := wireformat.ToKafka(message)
	if err != nil {
		return err
	}
